...
def run():
    ...
    for h in net.hosts:
        ...
        # 2. 清除 Mininet 自动生成的路由表 (必须，防止干扰)
        h.cmd("ip route flush table main")

        # 3. 【关键修复 A】给 h2 和 h3 的第二个接口 (eth1) 补上 IP
        # Mininet 默认只给 eth0 分配了 10.0.0.x，eth1 往往是没 IP 的

        # 4. 【关键修复 B】添加基础路由 (大家都有 eth0，默认往左走)
        # 这条规则解决了：h1->h2, h2->h1, h3->h2, h4->h3
        h.cmd("ip route add 10.0.0.0/24 dev %s-eth0" % h.name)

        # 5. 【关键修复 C】给中间节点添加“向右看”的路由
        # 必须显式告诉 h2 和 h3：去往右边的邻居，要走 eth1，别走 eth0！

        if h.name == "h2":
            # 告诉 h2: 去找 h3 (10.0.0.3) 请走 eth1
            h.cmd("ip route add 10.0.0.3 dev h2-eth1")

        if h.name == "h3":
            # 告诉 h3: 去找 h4 (10.0.0.4) 请走 eth1
